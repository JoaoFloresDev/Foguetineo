...
//        boxImg.layer.zPosition = -10
        let rotate = UIRotationGestureRecognizer(target: self, action: #selector(MenuViewController.rotate(_:)))
        self.view.addGestureRecognizer(rotate)
        
        let tap = UITapGestureRecognizer(target: self, action: #selector(MenuViewController.tap(_:)))
